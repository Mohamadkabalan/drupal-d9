...
namespace Drupal\custom_layout\Controller;
...
class ChooseBlockController implements ContainerInjectionInterface {
  ...
  public function inlineBlockList(SectionStorageInterface $section_storage, $delta, $region) {
    ...
    if (isset($blocks[$inline_blocks_category])) {
      ...
      $build['links']['#attributes']['class'][] = 'inline-block-list with-thumbnails';
  ...
  protected function getBlockLinks(SectionStorageInterface $section_storage, $delta, $region, array $blocks) {
    ...
    foreach ($blocks as $block_id => $block) {
      ...
      $module_path = $module_handler->getModule('custom_layout')->getPath();
      $filename = $module_path . '/thumbnails/' . strtolower(str_replace([' ', '/'], '_', $block['admin_label'])) . '.png';
      $thumbnail = '';

      //if (file_exists($filename)) {
        $thumbnail = '<img src="/' . $filename . '" />';
      //}

      $link = [
        'title' => Markup::create($thumbnail . '<span class="title">' . $block['admin_label'] . '</span>'),
        'url' => Url::fromRoute('layout_builder.add_block',
          [
            'section_storage_type' => $section_storage->getStorageType(),
            'section_storage' => $section_storage->getStorageId(),
